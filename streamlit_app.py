...
=====================================
Beautiful UI to demo the guardrails pipeline.
"""
...
with pipe_col:
    ...
    def render(container, icon, name, status, detail):
        ...
        badge_css = {"idle":"badge-idle","run":"badge-run","pass":"badge-pass","block":"badge-block"}[status]
        ...
        <div class="pipeline-card {css}">
            <div class="pipe-head">
                ...
                <span class="pipe-badge {badge_css}">{label}</span>
            </div>
...
if send and user_message.strip():
    ...
    try:
        ...
        if input_blocked:
            ...
            input_detail = " | ".join([c for c in checks if any(x in c for x in ["pii","injection","jailbreak","length_check: pass ("][:4])])
